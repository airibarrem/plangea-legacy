...
postprocess.results <- function(dir, outdir, scen,
                                quad.sd=TRUE, print.confidence=T,
                                hasweights=TRUE){
  
  # quad.sd stands for quadratic propagation of the uncertainties. If FALSE,
  # linear propagation is performed
  # print.confidence outputs the 95% confidence range (1.96 times a standard deviation)
  
  load(file=paste0(dir, "exrisk.t0.RData"))
  load(file=paste0(dir, "exrisk.t0.sd.RData"))
  
  exrisk.t0.sum = sum(exrisk.t0)
  exrisk.t0.sum.sd = ifelse(quad.sd, sqrt(sum(exrisk.t0.sd^2)), sum(exrisk.t0.sd))
  ...
  if (hasweights){
    ...
    exrisk.sum.red.cl <- rep(0, dim(weightm)[1])
    ...
    cb.total.sd <- rep(0, dim(weightm)[1])
    cb.total.cl <- rep(0, dim(weightm)[1])
    oc.total <- rep(0, dim(weightm)[1])
    oc.total.sd <- rep(0, dim(weightm)[1])
    oc.total.cl <- rep(0, dim(weightm)[1])
    ...
    for (w in 1:dim(weightm)[1]){
      ...
      exrisk.sum.sd <- ifelse(quad.sd,
                              sqrt(sum(res.exrisk.sd[,nsteps]^2)),
                              sum(res.exrisk.sd[,nsteps]))
      ...
      exrisk.sum.red.sd[w] <-  ifelse(quad.sd,
                                      sqrt((exrisk.t0.sum.sd^2) + (exrisk.sum.sd^2)),
                                      exrisk.t0.sum.sd + exrisk.sum.sd)
      ...
      cb.total.sd[w] <- ifelse(quad.sd,
                               ...
                               sum(cb.sd * res.total.restored.pu * A * const_cb))
      cb.total.cl[w] = 1.96 * cb.total.sd[w]
      oc.total[w] <- sum(oc * res.total.restored.pu * A * const_oc)
      oc.total.sd[w] <- ifelse(quad.sd,
                               sqrt(sum( (oc.sd^2) * ((res.total.restored.pu * A * const_oc)^2) )),
                               sum(oc.sd * res.total.restored.pu * A * const_oc))
      oc.total.cl[w] = 1.96 * oc.total.sd[w]
    ...
    if (print.confidence){
      df <- data.frame(scenario=rep(scen, dim(weightm)[1]), weightcb=weightm[,1],
                       ...
                       exrisk.sum.red.cl, cb.total, cb.total.cl, oc.total, oc.total.cl,
                       area.rest)      
    } else {
      df <- data.frame(scenario=rep(scen, dim(weightm)[1]), weightcb=weightm[,1],
                       weightbd=weightm[,2], exrisk.sum, exrisk.sum.red,
                       exrisk.sum.red.sd, cb.total, cb.total.sd, oc.total, oc.total.sd,
                       area.rest)
    }
    ...
    exrisk.sum.red.sd <-  ifelse(quad.sd,
                                 sqrt((exrisk.t0.sum.sd^2) + (exrisk.sum.sd^2)),
                                 exrisk.t0.sum.sd + exrisk.sum.sd)
    exrisk.sum.red.cl = 1.96 * exrisk.sum.red.sd
    ...
    cb.total.sd <- ifelse(quad.sd,
                          sqrt(sum( (cb.sd^2) * ((res.total.restored.pu * A * const_cb)^2) )),
                          sum(cb.sd * res.total.restored.pu * A * const_cb))
    cb.total.cl = 1.96 * cb.total.sd
    oc.total <- sum(oc * res.total.restored.pu * A * const_oc)
    oc.total.sd <- ifelse(quad.sd,
                          sqrt(sum( (oc.sd^2) * ((res.total.restored.pu * A * const_oc)^2) )),
                          sum(oc.sd * res.total.restored.pu * A * const_oc))
    oc.total.cl = 1.96 * oc.total.sd
    ...
    #                     cb.total, oc.total, area.rest)
    if (print.confidence){
      df <- data.frame(scenario=scen, weightbd=NA, weightcb=NA,
                       exrisk.sum, exrisk.sum.red, exrisk.sum.red.cl,
                       cb.total, cb.total.cl, oc.total, oc.total.cl,
                       area.rest)     
    } else {
      df <- data.frame(scenario=scen, weightbd=NA, weightcb=NA,
                       exrisk.sum, exrisk.sum.red, exrisk.sum.red.sd,
                       cb.total, cb.total.sd, oc.total, oc.total.sd,
                       area.rest)   
    }
  }
